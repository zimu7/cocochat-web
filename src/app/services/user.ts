...
export const userApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getContacts: builder.query<ContactResponse[], void>({
      ...
      async onQueryStarted(data, { dispatch, queryFulfilled }) {
        try {
          ...
          // 解析并写入备注
          users.forEach((c) => {
            const remark = c.contact_info.remark;
            if (remark) {
              dispatch(updateRemarkByUid({ uid: c.target_uid, remark }));
            }
          });
        } catch {
    ...
    // bot operations
    createBotAPIKey: builder.mutation<void, { uid: number; name: string; password?: string }>({
      query: ({ uid, name, password }) => ({
