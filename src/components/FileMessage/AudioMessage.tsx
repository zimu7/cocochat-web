...
const AudioMessage = ({ url, name, size, download }: Props) => {
  ...
  return (
    <div
      className={clsx(
        "md:w-96 flex flex-col gap-2 px-3 py-2 rounded-md border border-border dark:border-border overflow-hidden bg-stone-100 dark:bg-stone-900"
      )}
    >
      <div className="flex justify-between z-30 overflow-hidden">
        <div className="flex gap-2 ">
          <IconAudio className={clsx("w-9 h-auto")} />
          <div className="flex flex-col gap-1 text-sm text-foreground dark:text-foreground">
            <span title={name} className={clsx("font-bold truncate w-[240px]")}>
              {name}
            </span>
            <span className="text-muted-foreground">{_size}</span>
          </div>
        </div>
